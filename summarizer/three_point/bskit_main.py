...
def subbox_multiindex_to_index(multiindex, nsub_per_side):
    ...
    return int(
        multiindex[0] * nsub_per_side**2
        + multiindex[1] * nsub_per_side
        + multiindex[2]
    )
...
def field_subbox_pm(box_multiindex, nsub_per_side, source):
    ...
    sub_pm = ParticleMesh(
        BoxSize=source.BoxSize / nsub_per_side, Nmesh=source.Nmesh / nsub_per_side
    )
    ...
    # specified above)
    layout_src = source.pm.decompose(
        coords + start_coord, smoothing=0, transform=transform
    )

    # Find values of source grid in subregion, as 1d list matched to 1d coords list
    vals = source.readout(
        coords, resampler="nnb", transform=transform, layout=layout_src
    )
    ...
    return sub_field.pm.paint(
        coords, mass=vals, resampler="nnb", transform=sub_field.pm.affine_grid
    )
...
def field_subbox_slice(field, bounds):
    ...
    if ndim not in [2, 3]:
        raise NotImplementedError(
            "field_subbox_slice can only be used on 3D or 2D arrays"
        )
...
def k_field(box_size, n_mesh, kmin, kmax, p):
    ...
    if len(box_size) != 3:
        raise ValueError("box_size must be 3-element list!")
    if len(n_mesh) != 3:
        raise ValueError("n_mesh must be 3-element list!")

    # Function to apply to mesh, that sets modes to unity within bin and zero elsewhere
    def nbin_mask(k, v):
        # Compute |\vec{k}| values on the mesh
        kk = sum(ki**2.0 for ki in k) ** 0.5
    ...
    pm = ParticleMesh(
        BoxSize=np.ndarray.tolist(box_size), Nmesh=np.ndarray.tolist(n_mesh)
    )
    cf = ComplexField(pm)
...
def number_field(box_size, n_mesh, kmin, kmax):
    """Construct filtered grid of |k|^p values.
...
def pk_FFT(mesh, kmin, kmax):
    ...
    painted_real_field = number_field(
        mesh.attrs["BoxSize"], mesh.attrs["Nmesh"], kmin, kmax
    )
    ...
    Nbin_local = np.sum(painted_real_field**2.0) / np.prod(mesh.attrs["Nmesh"])
    ...
    painted_real_field = k_field(
        mesh.attrs["BoxSize"], mesh.attrs["Nmesh"], kmin, kmax, 0.5
    )
    k_mean_local = (
        np.sum(painted_real_field**2.0) / np.prod(mesh.attrs["Nmesh"]) / Nbin
    )
...
def geometric_k_mean(kmin, kmax):
...
def compute_Nbin(box_size, n_mesh, bins, verbose, return_number_fields=False):
    ...
    if verbose > 0 and comm.rank == 0:
        print("Starting Nbin computations")

    # Loop over k1,k2,k3 bins that define triangle bin
    for i in range(3):
        if verbose > 0 and comm.rank == 0:
            print("\tConstructing masked field for bin %d" % i)
        ...
        for j in range(i):
            if (np.abs(bins[i][0] - bins[j][0]) / bins[i][0] < tol) and (
                np.abs(bins[i][1] - bins[j][1]) / bins[i][1] < tol
            ):
    ...
    # From masked mesh, compute number of discrete modes within bin
    if verbose > 0 and comm.rank == 0:
        print("\tSumming over product of masked fields")
    Nbin_local = np.sum(
        number_fields[0] * number_fields[1] * number_fields[2]
    ) / np.prod(n_mesh)
    Nbin_sum = np.sum(comm.gather(Nbin_local))
    Nbin = comm.bcast(Nbin_sum)
...
def compute_k_means_on_grid(box_size, n_mesh, bins, verbose, Nbin, number_fields):
    ...
    if verbose > 0 and comm.rank == 0:
        print("Starting k_mean computations")
    ...
    for i in range(3):
        ...
        if (
            i == 0
            or (np.abs(bins[i][0] - bins[i - 1][0]) / bins[i][0] > tol)
            or (np.abs(bins[i][1] - bins[i - 1][1]) / bins[i][1] > tol)
        ):
            kk_field = k_field(box_size, n_mesh, bins[i][0], bins[i][1], 1.0)

        # Compute mean k1, k2, or k3 in triangle bin
        if i == 0:
            k_mean_local = (
                np.sum(kk_field * number_fields[1] * number_fields[2])
                / np.prod(n_mesh)
                / Nbin
            )
        elif i == 1:
            k_mean_local = (
                np.sum(number_fields[0] * kk_field * number_fields[2])
                / np.prod(n_mesh)
                / Nbin
            )
        elif i == 2:
            k_mean_local = (
                np.sum(number_fields[0] * number_fields[1] * kk_field)
                / np.prod(n_mesh)
                / Nbin
            )
...
def compute_bk_FFT_value(
    mesh, bins, Nbin=1, verbose=0, second_mesh=None, third_mesh=None
):
    ...
    if verbose > 0:
        print("Starting bispectrum computations")
    ...
    for i in range(3):
        ...
        if i == 0:
            ...
            if third_mesh is None and (
                (np.abs(bins[i][0] - bins[i - 1][0]) / bins[i][0] < tol)
                or (np.abs(bins[i][1] - bins[i - 1][1]) / bins[i][1] < tol)
            ):
    ...
    if verbose > 0 and comm.rank == 0:
        print("\tSumming over product of masked fields")
    B_local = (
        np.sum(delta_fields[0] * delta_fields[1] * delta_fields[2])
...
def bk_FFT_full(
    ...
    if second_mesh != None and not np.array_equal(
        mesh.attrs["BoxSize"], second_mesh.attrs["BoxSize"]
    ):
        raise ValueError("BoxSize mismatch between first and second mesh")
    if third_mesh != None and not np.array_equal(
        mesh.attrs["BoxSize"], third_mesh.attrs["BoxSize"]
    ):
        raise ValueError("BoxSize mismatch between first and third mesh")
    if second_mesh != None and not np.array_equal(
        mesh.attrs["Nmesh"], second_mesh.attrs["Nmesh"]
    ):
        raise ValueError("Nmesh mismatch between first and second mesh")
    if third_mesh != None and not np.array_equal(
        mesh.attrs["Nmesh"], third_mesh.attrs["Nmesh"]
    ):
    ...
    Nbin, number_fields = compute_Nbin(
        mesh.attrs["BoxSize"],
        mesh.attrs["Nmesh"],
        bins,
        verbose,
    ...
    if not approximate_k_means:
        # Compute explicitly
        k_mean = compute_k_means_on_grid(
            mesh.attrs["BoxSize"],
            mesh.attrs["Nmesh"],
            bins,
            verbose,
...
def bk_FFT_grid_info(mesh, bin0, bin1, bin2, verbose=0):
    ...
    k_mean = compute_k_means_on_grid(
        mesh.attrs["BoxSize"], mesh.attrs["Nmesh"], bins, verbose, Nbin, number_fields
    )

    return Nbin, k_mean[0], k_mean[1], k_mean[2]


def bk_FFT_unnormalized_value(
    mesh, bin0, bin1, bin2, verbose=0, second_mesh=None, third_mesh=None
):
    ...
    B = compute_bk_FFT_value(
        mesh,
        bins,
        Nbin=1,
        verbose=verbose,
        second_mesh=second_mesh,
        third_mesh=third_mesh,
    )

    return B


def bk_binned(mesh, bin0, bin1, bin2, verbose=0):
    ...
    if verbose > 0:
        print("Starting loop over slabs")
    for slab in SlabIterator(x3d, axis=0, symmetry_axis=symmetry_axis):
        ...
        ki = {}
        ki[0] = (
            slab.coords(0) * np.ones_like(slab.coords(1)) * np.ones_like(slab.coords(2))
        )
        ki[1] = (
            np.ones_like(slab.coords(0)) * slab.coords(1) * np.ones_like(slab.coords(2))
        )
        ki[2] = (
            np.ones_like(slab.coords(0)) * np.ones_like(slab.coords(1)) * slab.coords(2)
        )
    ...
    if verbose > 0:
        print("Starting loop over points in bins")
            ...
            for i2, k2 in enumerate(k_bin_arr[2]):
                ...
                if np.abs(k_tot_sq) < 1e-4 * kf:
                    if verbose > 1:
                        ...
                        print(
                            y3d_bin_arr[0][i0] * y3d_bin_arr[1][i1] * y3d_bin_arr[2][i2]
                        )
    ...
    # Return results
    if verbose > 0:
        print("B, Nbin = %g %g" % (B, Nbin))
    return B, Nbin, k_mean[0], k_mean[1], k_mean[2]


def generate_bin_edge_list(
    kmin=-1.0, kmax=-1.0, dk=-1.0, num_lowk_bins=0, dk_high=-1.0
):
    ...
    if dk <= 0.0:
        raise ValueError("dk must be > 0!")

    # Check for different dk at high k
    if num_lowk_bins > 0 and dk_high < 0.0:
...
def generate_equilateral_triangle_bin_list(
    kmin=-1.0, kmax=-1.0, dk=-1.0, num_lowk_bins=0, dk_high=-1.0, return_indices=False
):
...
def generate_squeezed_triangle_bin_list(
    kmin=-1.0,
    kmax=-1.0,
    dk=-1.0,
    ...
    if return_indices:
        ...
        tri_bin_indices = np.vstack(
            (squeezed_index_arr, short_index_arr, short_index_arr)
        ).T
        return tri_bin_indices
    else:
...
def generate_isosceles_triangle_bin_list(
    kmin=-1.0,
    kmax=-1.0,
    dk=-1.0,
    isos_mult=0,
    ...
    if return_indices:
        ...
        kL_index_arr = np.arange(len(kLU_arr))[
            nearest_index_arr[mask][:, 0].astype(int)
        ]
...
def generate_triangle_bin_list(
    kmin=-1.0,
    kmax=-1.0,
    dk=-1.0,
    mu_min=None,
    ...
    if num_fields not in [1, 2, 3]:
        raise ValueError("num_fields must be 1, 2, or 3!")
    ...
    def filter_triangles(arr):
        ...
        return np.array(
            list(
                filter(
                    lambda x: x[0][1] + x[1][1] >= x[2][0]
                    and x[0][1] + x[2][1] >= x[1][0]
                    and x[1][1] + x[2][1] >= x[0][0],
                    arr,
                )
            )
        )
    ...
    tri_bin_edges = np.sort(
        tri_arr_filt.view("f8,f8,f8,f8,f8,f8"), order=["f0", "f2", "f4"], axis=0
    ).view(np.float)
    ...
    if not return_indices:
        ...
        return np.array(
            [
                [
                    np.where(kLU_arr[:, 0] == tri_bin_edges[t][i])[0][0]
                    for i in [0, 2, 4]
                ]
                for t in range(len(tri_bin_edges))
            ]
        )
...
class FFTBispectrum(nbkfftpower.FFTBase):
    ...
    ----------
    mesh : MeshSource
        already generated mesh source from the catalog
    ...
    def __init__(
        ...
        if third is not None:
            if second is None:
                raise ValueError(
                    "Need second source defined if third source is defined!"
                )
            self.third = nbkfftpower._cast_source(third, Nmesh=Nmesh, BoxSize=BoxSize)
            self.num_fields += 1
        # else:
        #     # If mesh is specified, just set it as the first mesh
        #     self.mesh = mesh
        #     self.second = second
        #     self.third = third
        #     self.num_fields = 1
        ...
        if k_edges is not None:
            ...
            if triangle_type == "all":
                self.k_edges = generate_triangle_bin_list(
                    kmin=kmin,
                    kmax=kmax,
                    dk=dk,
                    dmu=dmu,
                    ...
                    num_lowk_bins=num_lowk_bins,
                    dk_high=dk_high,
                )
                self.k_indices = generate_triangle_bin_list(
                    kmin=kmin,
                    kmax=kmax,
                    dk=dk,
                    dmu=dmu,
                ...
                self.k_edges = generate_equilateral_triangle_bin_list(
                    kmin=kmin,
                    kmax=kmax,
                    dk=dk,
                    num_lowk_bins=num_lowk_bins,
                ...
                self.k_indices = generate_equilateral_triangle_bin_list(
                    kmin=kmin,
                    kmax=kmax,
                    dk=dk,
                    num_lowk_bins=num_lowk_bins,
                ...
                self.k_edges = generate_squeezed_triangle_bin_list(
                    kmin,
                    kmax,
                    dk,
                ...
                self.k_indices = generate_squeezed_triangle_bin_list(
                    kmin,
                    kmax,
                    dk,
                    squeezed_bin_index=squeezed_bin_index,
                ...
                self.k_edges = generate_isosceles_triangle_bin_list(
                    kmin,
                    kmax,
                    dk,
                    isos_mult=isos_mult,
                ...
                self.k_indices = generate_isosceles_triangle_bin_list(
                    kmin,
                    kmax,
                    dk,
                    isos_mult=isos_mult,
    ...
    def measure_bispectrum(
        ...
        for i, tri in enumerate(tri_list):
            ...
            # if necessary
            if meas_type == "full":
                Bi, Ntri, k1m, k2m, k3m = bk_FFT_full(
                    self.mesh,
                    tri_list[i][0:2] * unit_fac,
                    tri_list[i][2:4] * unit_fac,
                    tri_list[i][4:6] * unit_fac,
                    second_mesh=self.second,
                    third_mesh=self.third,
                )
            elif meas_type == "grid_info":
                Ntri, k1m, k2m, k3m = bk_FFT_grid_info(
                    self.mesh,
                    tri_list[i][0:2] * unit_fac,
                    tri_list[i][2:4] * unit_fac,
                    tri_list[i][4:6] * unit_fac,
        ...
        b_return = {
            "index": self.b["index"][-len(tri_list) :],
    ...
    def measure_bispectrum_faster(
        ...
        k_bin_edges = generate_bin_edge_list(
            self.attrs["kmin"],
            self.attrs["kmax"],
            self.attrs["dk"],
        ...
        delta_fields = {}
        if verbose > 0 and comm.rank == 0:
            print("Starting delta_field computations")
        ...
        if verbose > 0 and comm.rank == 0:
            print("\tStarting sum over triangles")
        ...
        for t, triple in enumerate(triple_index_list):
            ...
            B_local = (
                np.sum(
                    delta_fields[triple[0]]
                    * delta_fields[triple[1]]
                    * delta_fields[triple[2]]
                )
                * self.mesh.attrs["BoxSize"].prod() ** 2
                / np.prod(self.mesh.attrs["Nmesh"])
            )  # / Nbin
            B_sum = np.sum(comm.gather(B_local))
            B = comm.bcast(B_sum)

            # Convert units of B value to h^-1 Mpc^6
            B *= self.attrs["pos_units_mpcoverh"] ** 6.0

            # Add measurement info to stored arrays of previous measurements
            if self.b is None:
                self.b = {
                    "index": np.array([triangle_index_list[t]]),
                ...
                self.b["index"] = np.append(
                    self.b["index"], [triangle_index_list[t]], axis=0
                )
                self.b["k_edge"] = np.append(
                    self.b["k_edge"], [triangle_bin_edge_list[t]], axis=0
                )
                self.b["k_mean"] = np.append(self.b["k_mean"], [0.0], axis=0)
            ...
            if comm.rank == 0:
                if verbose > 0:
                    print(
                        "FFT Bk: %d %d %g %g %g %g %g"
                        % (
                            t,
                            triangle_index_list[t],
                            self.b["k_mean"][-1][0],
                            self.b["k_mean"][-1][1],
                            self.b["k_mean"][-1][2],
                            self.b["B"][-1],
                        ...
                        with open(out_file, "a") as f:
                            for tt in range(num_tri_to_write):
                                f.write(
                                    ...
                                    % (
                                        triangle_index_list[
                                            t - (num_tri_to_write - 1) + tt
                                        ],
                                        triangle_bin_edge_list[t - 99 + tt][0],
                                        triangle_bin_edge_list[
                                            t - (num_tri_to_write - 1) + tt
                                        ][1],
                                        triangle_bin_edge_list[t - 99 + tt][2],
                                        triangle_bin_edge_list[
                                            t - (num_tri_to_write - 1) + tt
                                        ][3],
                                        triangle_bin_edge_list[t - 99 + tt][4],
                                        triangle_bin_edge_list[
                                            t - (num_tri_to_write - 1) + tt
                                        ][5],
                                        self.b["B"][-num_tri_to_write + tt],
                                    )
                                )
                ...
                for tt in range(t_left):
                    f.write(
                        ...
                        % (
                            ...
                            triangle_bin_edge_list[t - t_left + 1 + tt][1],
                            triangle_bin_edge_list[t - t_left + 1 + tt][2],
                            triangle_bin_edge_list[t - t_left + 1 + tt][3],
                            triangle_bin_edge_list[t - t_left + 1 + tt][4],
                            triangle_bin_edge_list[t - t_left + 1 + tt][5],
                            self.b["B"][-t_left + tt],
                        )
                    )
        ...
        # Return dict of measurements made in this function call
        b_return = {
            "index": self.b["index"][-len(triangle_index_list) :],
    ...
    def measure_gridinfo_faster(
        ...
        for i in k_bin_indices:
            ...
            number_fields[i] = number_field(
                box_size, n_mesh, k_bin_edges[i][0], k_bin_edges[i][1]
            )
            kk_fields[i] = k_field(
                box_size, n_mesh, k_bin_edges[i][0], k_bin_edges[i][1], 1.0
            )
        ...
        for t, triple in enumerate(triple_index_list):
            ...
            Nbin_local = np.sum(
                number_fields[triple[0]]
                * number_fields[triple[1]]
                * number_fields[triple[2]]
            ) / np.prod(n_mesh)
            Nbin_sum = np.sum(comm.gather(Nbin_local))
            Nbin = comm.bcast(Nbin_sum)
                            ...
                            for tt in range(num_tri_to_write):
                                f.write(
                                    ...
                                    % (
                                        ...
                                        self.b["N_tri"][-num_tri_to_write + tt],
                                    )
                                )
                        if verbose > 0:
                            print(
                                "Wrote %d B values, starting with triangle index %d"
                                % (
                                    num_tri_to_write,
                                    triangle_index_list[
                                        t - (num_tri_to_write - 1) + tt
    ...
    def save_bispectrum(self, out_file):
        ...
        header = (
            "k1_mean k2_mean k3_mean k1_low k1_high k2_low k2_high k3_low k3_high "
